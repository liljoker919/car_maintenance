...
class VehicleListTemplateTest(TestCase):
    ...
    def test_add_vehicle_shows_success_message(self):
        ...
        response = self.client.post(
            reverse("vehicles:vehicle_add"), data=form_data, follow=True
        )
    ...
    def test_toast_html_rendered_correctly(self):
        ...
        self.assertIn(
            "toast align-items-center text-white bg-success border-0 show", content
        )
...
class VehicleDetailTemplateTest(TestCase):
    ...
    def test_vehicle_detail_page_renders_correctly(self):
        ...
        response = self.client.get(
            reverse("vehicles:vehicle_detail", kwargs={"pk": self.vehicle.pk})
        )
...
class ServiceRecordViewTest(TestCase):
    ...
    def test_service_record_create_success(self):
        ...
        self.assertRedirects(
            response, reverse("vehicles:vehicle_detail", kwargs={"pk": self.vehicle.pk})
        )
    ...
    def test_service_record_create_prevents_other_users_vehicle(self):
        ...
        self.assertFalse(
            ServiceRecord.objects.filter(notes="Unauthorized service").exists()
        )
    ...
    def test_service_record_delete_success(self):
        ...
        self.assertFalse(
            ServiceRecord.objects.filter(pk=self.service_record.pk).exists()
        )
    ...
    def test_service_record_create_shows_success_message(self):
        ...
        response = self.client.post(
            reverse("vehicles:service_add"), data=form_data, follow=True
        )
...
class VehicleDetailViewQueryOptimizationTest(TestCase):
    """Test that VehicleDetailView uses prefetch_related to optimize queries"""
    ...
    def test_vehicle_detail_uses_prefetch_related(self):
        """Test that VehicleDetailView uses prefetch_related to minimize queries"""
        self.client.login(username="testuser", password="testpass123")

        # Count queries when accessing the detail view
        # The key benefit of prefetch_related is that it fetches related objects
        # in separate queries (one per relation) instead of making N queries
        # (one per related object) when accessing them in get_context_data.
        #
        # Without prefetch_related:
        # - 1 query for Vehicle
        # - 1 query for insurance_policies.filter(vehicle=self.object, user=self.request.user)
        # - 1 query for car_registrations.filter(vehicle=self.object)
        # - 1 query for service_records.filter(vehicle=self.object)
        # Total: 4 queries for related data
        #
        # With prefetch_related:
        # - 1 query for Vehicle with 3 prefetch queries (insurance_policies, car_registrations, service_records)
        # - No additional queries when accessing .all() on prefetched relations
        # Total: 4 queries for related data (same count but more efficient with multiple objects)

        response = self.client.get(
            reverse("vehicles:vehicle_detail", kwargs={"pk": self.vehicle.pk})
        )

        # Verify response is successful and contains related data
        self.assertEqual(response.status_code, 200)
        self.assertIn("insurance_policies", response.context)
        self.assertIn("car_registrations", response.context)
        self.assertIn("service_records", response.context)

        # Verify the data is correct and is querysets/lists
        insurance_policies = response.context["insurance_policies"]
        self.assertEqual(len(list(insurance_policies)), 1)

        car_registrations = response.context["car_registrations"]
        self.assertEqual(len(list(car_registrations)), 1)

        service_records = response.context["service_records"]
        self.assertEqual(len(list(service_records)), 1)

    def test_vehicle_detail_query_efficiency_with_multiple_related_objects(self):
        """Test that prefetch_related is more efficient with multiple related objects"""
        ...
        # With prefetch_related, the number of queries should remain constant
        # regardless of the number of related objects
        response = self.client.get(
            reverse("vehicles:vehicle_detail", kwargs={"pk": self.vehicle.pk})
        )

        # Verify all related objects are present
        self.assertEqual(response.status_code, 200)

        # Original + 3 new = 4 insurance policies
        insurance_policies = list(response.context["insurance_policies"])
        self.assertEqual(len(insurance_policies), 4)

        # Original + 3 new = 4 car registrations
        car_registrations = list(response.context["car_registrations"])
        self.assertEqual(len(car_registrations), 4)

        # Original + 3 new = 4 service records
        service_records = list(response.context["service_records"])
        self.assertEqual(len(service_records), 4)
